...
const LAYER_NODE_MODULES_DIR_NAME = "node_dependencies";
...
export function nodeModulesAdjust(): void {
  ...
  execSync("npm init -y");
  execSync("npm install mongoose uuid dotenv nodemailer busboy aws-sdk date-fns");

  console.log(`Node modules installed at ${nodeModulesDirectory}`);
  ...
  for (const file of files) {
    if (file !== NODE_MODULES_DIR_NAME) {
      fs.rmSync(path.join(destinationDirectory, file), {
        recursive: true,
        force: true,
      });
    }
