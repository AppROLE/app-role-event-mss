...
export class UploadGalleryEventUseCase {
    ...
    async execute(eventId: string, eventPhoto: Buffer, typePhoto: string, mimetype: string) {
        ...
        await this.eventRepo.updateGalleryArray(eventName, `${Environments.getEnvs().cloudFrontUrl}/${imageKey}`);
        
        await this.fileRepo.uploadEventGalleryPhoto( eventName, imageKey, eventPhoto, mimetype);
